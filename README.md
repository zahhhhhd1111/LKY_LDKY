...
# ZYY：
1、项目红线交林草湿一张图后空间连接保护地数据；2、检查DILEI与DLMC，添加相关备注到BZ字段；3、可能会存在BZ_1以及其他无关字段，现在无需手动删除；4、擦除历史红线
注：ZYY仅需处理上述内容，命名为“多县ZYY空间连接保护区”，存入工程目录下的“输出数据”数据库即可。
...
- `6-写报告.py`：按县执行报告小任务（使用林地申请说明、被使用林地单位明细表等），输出 Markdown。
- `project_config.py`：项目路径、源数据名、模板目录等配置。
- `ZYY字段属性标准设置.MD`：字段值翻译、县名/投影带、乡镇/村代码等标准表。
- `修改记录_标准ZYY字段.txt`：字段标准修改记录。
- `可研附表模板.xlsx`：5 号脚本填充的可研附表源模板。
